# Matrices del modelo

**Contenido generado.** Lo escribe `python3 scripts/verificar_modelo.py`
desde el curso construido. No se edita a mano: si algo aquí no cuadra, lo que
hay que corregir es el generador o el modelo, no esta tabla.

Curso: 11 unidades · 53 competencias · 1032 ejercicios · 1791 pasos.

## M2 · qué dimensión acredita cada tipo de ejercicio

Pasos publicados por tipo y dimensión.

| Tipo | compr. | recon. | recup. | transf. | reten. | oído |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Acento léxico | 2 | 0 | 2 | 0 | 0 | 0 |
| Comprensión lectora | 37 | 0 | 5 | 0 | 0 | 0 |
| Contraste semántico | 32 | 32 | 32 | 0 | 0 | 0 |
| Crítica de retrotraducción | 30 | 0 | 30 | 0 | 0 | 0 |
| Diagnóstico y reparación | 86 | 0 | 86 | 0 | 0 | 0 |
| Dictado | 0 | 0 | 0 | 0 | 0 | 104 |
| Elección en contexto | 0 | 101 | 101 | 0 | 0 | 0 |
| Escritura cirílica | 0 | 0 | 5 | 0 | 0 | 0 |
| Examen | 1 | 0 | 55 | 0 | 0 | 0 |
| Hueco | 0 | 0 | 42 | 0 | 0 | 0 |
| Inducción de regla | 10 | 0 | 0 | 10 | 0 | 0 |
| Producción | 0 | 0 | 44 | 0 | 0 | 0 |
| Traducción es → ru | 0 | 0 | 43 | 0 | 0 | 0 |
| Traducción ru → es | 44 | 0 | 0 | 0 | 0 | 0 |
| Transferencia guiada | 68 | 0 | 30 | 68 | 0 | 0 |
| Transformación | 0 | 0 | 40 | 0 | 0 | 0 |
| Vacío de información | 0 | 0 | 0 | 22 | 0 | 0 |
| Vocabulario | 186 | 211 | 232 | 0 | 0 | 0 |
| **total** | 496 | 344 | 747 | 100 | 0 | 104 |

La columna de retención diferida está vacía a propósito: no es una tarea, es
una condición temporal sobre la recuperación (modelo, M1).

## M1 · cobertura de las competencias por dimensión

| Dimensión | Competencias cubiertas | Estado |
| --- | ---: | --- |
| `comprension_explicita` | 53/53 | canal de reconocimiento: 53/53 |
| `reconocimiento_escrito` | 48/53 | canal de reconocimiento: 53/53 |
| `recuperacion_escrita` | 53/53 | canal de producción: 53/53 |
| `transferencia_contextual` | 39/53 | no garantizada: sólo donde el material trae escena nueva |
| `retencion_diferida` | 0/53 | sin ejercicio propio, por definición |
| `reconocimiento_auditivo` | 53/53 | canal de escucha: 53/53 |

La exigencia del modelo es por **canal**, no por dimensión suelta: hay
competencias que no admiten elección de formas —«distinguir grafemas
cirílicos de homógrafos latinos» no puede ofrecerlas, porque en pantalla
son la misma imagen— y exigirla obligaría a inventar el ejercicio que el
material no da.

## M5 · tamaño y cobertura del examen

| Unidad | Competencias | Ítems | Pasos |
| --- | ---: | ---: | ---: |
| 001 | 5 | 14 | 16 |
| 002 | 4 | 15 | 16 |
| 003 | 5 | 15 | 17 |
| 004 | 5 | 16 | 17 |
| 005 | 5 | 17 | 19 |
| 006 | 5 | 15 | 18 |
| 007 | 4 | 13 | 14 |
| 008 | 5 | 16 | 18 |
| 009 | 5 | 16 | 18 |
| 010 | 5 | 15 | 17 |
| 011 | 5 | 16 | 17 |

## M4 · escalera de intervalos

| Escalón | 0 | 1 | 2 | 3 | 4 | 5 | 6 |
| --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| Días | 1 | 3 | 7 | 16 | 35 | 70 | 140 |

El escalón sube una vez por ocasión de repaso, nunca una vez por
ejercicio: si no, una sola sesión llevaría la competencia al último
escalón y no volvería a verse en cuatro meses.
